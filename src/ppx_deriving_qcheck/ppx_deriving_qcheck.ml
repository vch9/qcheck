...
(** [is_rec_type_decl typ_names typ] looks for elements of [typ_names]
    recursively in [typ]. *)
let rec is_rec_type_decl typ_names typ =
  let in_type_kind =
...
and is_rec_typ typ_names = function
...
and is_rec_constr_decl typ_names cd =
...
(** [gen lg args] creates a generator using [lg].
...
(** [sized typ_name is_rec to_gen xs] uses [is_rec] to determine recursive
    ...
    variants. *)
let sized ~loc (is_rec : 'a -> bool) (to_gen : 'a -> expression) (xs : 'a list) =
  let (module A) = Ast_builder.make loc in
  let leaves =
...
(** [record loc gens ?f label_decls] transforms [gens] and [label_decls] to
    ...
    Similarly to {!tuple}, we can use [f] to transform records, for instance:
    {[
  ...
  let to_gen (row : row_field) : expression =
    ...
    let gen =
      ...
      | Rtag (label, _, typs) ->
          ...
          tuple ~loc ~f (List.map (gen_from_type ~loc) typs)
    in
    [%expr [%e w], [%e gen]]
  in
  let gen = sized ~loc is_rec to_gen rws in
...
let gen_from_kind_variant ~loc xs =
  let (module A) = Ast_builder.make loc in
  let is_rec cd = is_rec_constr_decl (curr_types ()) cd in
  sized ~loc is_rec (gen_from_constr ~loc) xs
...
(** [gen_from_type_declaration loc td] creates a generator from the type declaration.
    ...
    The derivated generator is not recursive, we return only the generator.

    - `Recursive of expression * expression
    The derivated generator was recursive (i.e. val gen : n -> t Gen.t), we return
...
let gen_from_type_declaration ~loc td =
  ...
  let gen =
    ...
    | Ptype_variant xs -> gen_from_kind_variant ~loc xs
    | Ptype_record xs ->
...
(** [derive_gen ~loc xs] creates generators for type declaration in [xs].
    ...
    He also has an hidden purpose: he sets the environment prior to
    the derivation. He identifies recursive types declarations (e.g list)
